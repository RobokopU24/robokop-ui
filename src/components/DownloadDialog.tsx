  ...
  const publications = edgeIds
    .flatMap((edgeId) =>
      message.knowledge_graph.edges[edgeId].attributes
        .filter(
          (attr: { attribute_type_id: string }) =>
            attr.attribute_type_id === 'biolink:publications',
        )
...
export default function DownloadDialog({
  ...
  const [fileName, setFileName] = React.useState(
    `ROBOKOP_${download_type === 'all_queries' ? 'query' : 'answer'}`,
  )
  ...
  return (
    <Dialog open={open} onClose={handleClose} aria-labelledby='alert-dialog-title'>
      <DialogTitle id='alert-dialog-title'>
        Download {download_type === 'all_queries' ? 'Query' : 'Answer'}
      </DialogTitle>
