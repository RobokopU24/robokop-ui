...
import { Accordion, AccordionSummary, Typography, AccordionDetails, Box, List, ListItem, ListItemText } from "@mui/material";
...
function TemplateModal({ isOpen, onClose, onCancel }: ExampleModalProps) {
  ...
  const templateQueries = (examples as unknown as TemplatesArray).filter((example) => example.type === "template");
  ...
  return (
    <Modal open={isOpen} onClose={onModalClose}>
      ...
      >
        ...
        <p style={{ color: "#5E5E5E", fontSize: "14px", margin: "8px 0 0 0" }}>Use any of the customizable templates available from the list</p>
        {templateQueries.map((query, index) => (
          ...
          >
            <AccordionSummary aria-controls={`panel${index + 1}d-content`} id={`panel${index + 1}d-header`} expandIcon={<ExpandMoreIcon />} sx={{ m: 0, p: 0, borderRadius: "8px", border: "none" }} onClick={() => handleSelectExample(query)}>
                ...
                {query.sub_examples &&
                  query.sub_examples.map((subExample, subIndex) => (
                    <ListItem key={subIndex} sx={{ p: 0, ml: 2, borderBottom: "1px solid #e0e0e0" }}>
                      <ListItemText sx={{ p: 0, m: 0 }} primary={<SubExample subExample={subExample} mainNodesTemplate={query.template as TemplatePart[]} />} />
                    </ListItem>
                  ))}
              </List>
            </AccordionDetails>
