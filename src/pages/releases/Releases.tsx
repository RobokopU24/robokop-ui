...
import { Button, Container, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, Skeleton, Stack, Typography } from "@mui/material";
import AddIcon from "@mui/icons-material/Add";
import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";

import { createRelease, deleteRelease, getReleaseNotes, GitHubRelease, updateRelease } from "./functions";
...
function Releases() {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, values }: { id: number; values: ReleaseFormValues }) => updateRelease(id, values),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["release-notes"] });
  ...
  return (
    <Container maxWidth="md" sx={{ my: 6 }}>
      ...
      {isLoadingReleases ? (
        ...
        <Stack spacing={4}>
          {releaseNotes.map((releaseNote) => (
            <ReleaseNoteCard key={releaseNote.id} releaseNote={releaseNote} onEdit={hasAccess ? openEditDialog : undefined} onDelete={hasAccess ? confirmDelete : undefined} />
          ))}
        </Stack>
      ) : (
      ...
      <ReleaseFormDialog open={formOpen} onClose={closeFormDialog} onSubmit={handleFormSubmit} isSubmitting={createMutation.isPending || updateMutation.isPending} existingRelease={editingRelease} />

      <Dialog open={Boolean(deleteTarget)} onClose={cancelDelete}>
        <DialogTitle>Delete Release</DialogTitle>
        <DialogContent>
          <DialogContentText>
            Are you sure you want to delete <strong>{deleteTarget?.name || deleteTarget?.tagName}</strong>? This action cannot be undone.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
