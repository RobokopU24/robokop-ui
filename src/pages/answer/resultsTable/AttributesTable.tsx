...
const ValueCell: React.FC<{ value: string | string[] }> = ({ value }) => (
  <TableCell>
    <ul style={{ padding: 0, margin: 0, listStyleType: "none" }}>{Array.isArray(value) ? value.map((valueItem, valueItemIndex) => <li key={valueItemIndex}>{valueItem}</li>) : <li>{value}</li>}</ul>
  </TableCell>
);
...
const PublicationLinkCell: React.FC<{ value: string | string[] }> = ({ value }) => {
  ...
  return (
    <TableCell>
      ...
      {isSummaryModalOpen && <SummaryModal isOpen={isSummaryModalOpen} onModalClose={() => setIsSummaryModalOpen(false)} links={linkList as string[]} ids={idList as string[]} />}
