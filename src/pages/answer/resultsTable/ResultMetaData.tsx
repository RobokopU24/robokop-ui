...
import SummarizeKGNodesWithAIModal from './SummarizeKGNodesWithAIModal'
...
export default function ResultMetaData({ metaData, result }: ResultMetaDataProps) {
  ...
  return (
    <Paper id='resultMetaData' elevation={3}>
      <div>
        <h4>Summarize Results with AI</h4>
        <Button
          variant='outlined'
          size='small'
          sx={{ mb: 2 }}
        ...
        <SummarizeKGNodesWithAIModal
          isOpen={isSummarizeModalOpen}
          onModalClose={() => setIsSummarizeModalOpen(false)}
          minimalJson={minifiedJson}
          isLoggedIn={!!user}
          canSummarize={canAccess('kg-nodes-summary')}
        />
      </div>
      <div>
