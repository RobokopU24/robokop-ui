...
export default function useAnswerStore() {
  ...
  function averageAnalysesScores(msg) {
    const resultsWithScore = msg.results.map((result) => {
      ...
      const score =
        result.analyses.reduce((sum, analysis) => sum + analysis.score, 0) / result.analyses.length;
      return {
        ...result,
        score,
      };
      ...
      row.analyses.forEach((analysis) => {
        ...
        if (Array.isArray(analysis.support_graphs)) {
          analysis.support_graphs.forEach((supportGraphId) => {
            support_graph_edge_bindings.push(
              ...message.auxiliary_graphs[supportGraphId].edges.map((e) => ({ id: e }))
            );
          });
        }
        ...
        [...edge_bindings, ...support_graph_edge_bindings].forEach((binding) => {
          ...
          if (kgEdge) {
            ...
            const edgeKey = `${subjectNode.name || kgEdge.subject} ${stringUtils.displayPredicate(
              kgEdge.predicate
            )} ${objectNode.name || kgEdge.object}`;
