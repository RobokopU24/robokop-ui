      ...
      if (association.slotUsage && !association.abstract) {
        ...
        const qualifiers = Object.entries(association.slotUsage)
          .map(([qualifierName, properties]) => {
            ...
            let subpropertyOf;
            if (properties.subproperty_of && model.slots.lookup.has(properties.subproperty_of)) {
...
export default function TextEditorRow({ row, index }) {
  ...
  const subject = (
    (query_graph.nodes[edge.subject].categories && query_graph.nodes[edge.subject].categories[0]) ||
    'biolink:NamedThing'
  )
    .replace('biolink:', '')
    .match(/[A-Z][a-z]+/g)
    ...
    .toLowerCase();
  const predicate = (
    (query_graph.nodes[edge.subject].categories && edge.predicates[0]) ||
    'biolink:related_to'
  )
    .replace('biolink:', '')
    .replace(/_/g, ' ');
  const object = (
    (query_graph.nodes[edge.object].categories && query_graph.nodes[edge.object].categories[0]) ||
    'biolink:NamedThing'
  )
    .replace('biolink:', '')
    .match(/[A-Z][a-z]+/g)
