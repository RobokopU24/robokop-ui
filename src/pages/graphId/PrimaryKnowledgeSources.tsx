...
function PrimaryKnowledgeSources({
  graphData,
  schema,
}: {
  graphData?: any
  schema: GraphSchemaV2
}) {
  ...
  const data = useMemo(() => {
    return Object.entries(schema?.schema.edges_summary.primary_knowledge_sources || {}).map(
      ([knowledgeSource, count]) => {
        // Get all predicates for this knowledge source from predicates_by_knowledge_source
        const predicatesForSource =
          schema?.schema.edges_summary.predicates_by_knowledge_source?.[knowledgeSource] || {}
        const predicateArray = Object.entries(predicatesForSource).map(
