...
function EditModal({ isOpen, onClose, selectedUser }: EditModalProps) {
  ...
  const updateRoleMutation = useMutation({
    mutationFn: ({ userId, newRoleId }: { userId: string; newRoleId: number }) =>
      updateUsersRole([userId], newRoleId),
    onSuccess: () => {
    ...
    <>
      <Dialog open={isOpen} onClose={onClose} maxWidth='sm' fullWidth>
        ...
        <DialogActions>
          ...
          <Button
            onClick={handleSave}
            disabled={!hasChanges || updateRoleMutation.isPending || rolesLoading}
            variant='contained'
          >
