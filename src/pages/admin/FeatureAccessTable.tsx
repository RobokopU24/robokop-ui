...
import { getFeatures, updateFeatureRoles, createFeature, deleteFeatures, Feature, CreateFeaturePayload } from "../../functions/featureFunctions";
...
function FeatureAccessTable() {
  ...
  const handleNewFeatureChange = (field: keyof CreateFeaturePayload) => (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <div>
                  ...
                  <TableCell>
                    {isEditing ? (
                      ...
                      <Box sx={{ display: "flex", gap: 1 }}>
                        ...
                        <IconButton onClick={() => handleOpenDeleteDialog(feature)} color="error" size="small">
                          <DeleteIcon />
                        </IconButton>
                      </Box>
                    )}
      ...
      <Dialog open={addDialogOpen} onClose={handleCloseAddDialog} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Box sx={{ display: "flex", flexDirection: "column", gap: 2, mt: 1 }}>
            <TextField label="Feature ID" value={newFeature.featureId} onChange={handleNewFeatureChange("featureId")} placeholder="e.g., my_feature" required fullWidth helperText="Unique identifier for the feature (used in code)" />
            <TextField label="Feature Name" value={newFeature.featureName} onChange={handleNewFeatureChange("featureName")} placeholder="e.g., My Feature" required fullWidth helperText="Human-readable name for the feature" />
            <TextField label="Description" value={newFeature.description} onChange={handleNewFeatureChange("description")} placeholder="Describe what this feature does..." multiline rows={3} fullWidth />
        ...
        <DialogActions>
          ...
          <Button onClick={handleAddFeature} variant="contained" disabled={!isAddFormValid || createMutation.isPending} startIcon={createMutation.isPending ? <CircularProgress size={16} /> : null}>
      ...
      <Dialog open={deleteDialogOpen} onClose={handleCloseDeleteDialog} maxWidth="sm">
        ...
        <DialogContent>
          <DialogContentText>
            Are you sure you want to delete the feature <strong>{featureToDelete?.featureName}</strong>? This action cannot be undone.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
