          ...
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, alignItems: 'center' }}>
            {role.features.length === 0 ? (
              ...
              <>
                ...
                />
                {role.features.length > 1 && (
                  <Chip
                    label={`+${role.features.length - 1} more`}
                    size='small'
                    variant='outlined'
                    color='default'
                  />
...
function RolesTable() {
  ...
  const handleNewRoleChange =
    (field: keyof CreateRolePayload) => (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleEditRoleChange =
    (field: keyof UpdateRolePayload) => (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <div>
      ...
      <Dialog open={deleteDialogOpen} onClose={handleCloseDeleteDialog} maxWidth='sm'>
        ...
        <DialogContent>
          <DialogContentText>
            Are you sure you want to delete the role <strong>{roleToDelete?.roleName}</strong>? This
        ...
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
            <TextField
              ...
              onChange={handleNewRoleChange('roleName')}
                ...
                renderValue={(selected) => (
                  <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                    {selected.map((featureId) => {
                      ...
                      return feature ? (
                        <Chip key={featureId} label={feature.featureName} size='small' />
                      ) : null
                    })}
                  </Box>
                )}
        ...
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
            <TextField
              ...
              onChange={handleEditRoleChange('roleName')}
              placeholder='e.g., moderator'
              required
              fullWidth
              helperText='Name for the role'
            ...
            <TextField
              ...
              onChange={handleEditRoleChange('description')}
              placeholder='Describe what this role can do...'
              multiline
              rows={3}
              fullWidth
